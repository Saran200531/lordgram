    ...
    <article className="bg-white rounded-[2.5rem] shadow-2xl shadow-slate-200/50 border border-slate-50 overflow-hidden group hover:shadow-violet-100 transition-all duration-500">
      ...
      <div className="relative aspect-square md:aspect-[4/5] w-full bg-slate-50 overflow-hidden">
        <img 
          ...
          onDoubleClick={() => { setLiked(true); setReaction('❤️'); }}
        />
          ...
          <div className="flex items-center gap-6">
            <div className="relative">
              <button 
                ...
                className={`flex items-center gap-1.5 transition-all ${liked ? 'text-pink-500 scale-110' : 'text-slate-300 hover:text-pink-500 hover:scale-110'}`}
              >
                <Icons.Heart className="w-7 h-7" filled={liked} />
                <span className="text-xs font-black">{post.likes + (liked ? 1 : 0)}</span>
              </button>
              ...
              {showReactions && (
                ...
                >
                  {emojis.map(e => (
                    <button key={e} onClick={() => { setReaction(e); setLiked(true); setShowReactions(false); }} className="hover:scale-150 transition-transform text-xl">{e}</button>
                  ))}
                </div>
              )}
            </div>
